...
/**
 * Convert Java SE 8 {@code OptionalLong}.
 * <p>
...
final class OptionalLongStringConverter<T> implements TypedStringConverter<T> {
    ...
    static {
        OptionalLongStringConverter<?> instance;
        try {
            Class<?> cls = RenameHandler.INSTANCE.loadType("java.util.OptionalLong");
            Method methodPresent = cls.getDeclaredMethod("isPresent");
            Method methodGet = cls.getDeclaredMethod("getAsLong");
            Method methodOf = cls.getDeclaredMethod("of", Long.TYPE);
            Method methodEmpty = cls.getDeclaredMethod("empty");
            Object empty = methodEmpty.invoke(null);
            instance = new OptionalLongStringConverter(cls, methodPresent, methodGet, methodOf, empty);
        } catch (Exception ex) {
            instance = null;
        }
        INSTANCE = instance;
