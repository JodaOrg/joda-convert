...
/**
 * Convert Java SE 8 {@code OptionalDouble}.
 * <p>
...
final class OptionalDoubleStringConverter<T> implements TypedStringConverter<T> {
    ...
    static {
        OptionalDoubleStringConverter<?> instance;
        try {
            Class<?> cls = RenameHandler.INSTANCE.loadType("java.util.OptionalDouble");
            Method methodPresent = cls.getDeclaredMethod("isPresent");
            Method methodGet = cls.getDeclaredMethod("getAsDouble");
            Method methodOf = cls.getDeclaredMethod("of", Double.TYPE);
            Method methodEmpty = cls.getDeclaredMethod("empty");
            Object empty = methodEmpty.invoke(null);
            instance = new OptionalDoubleStringConverter(cls, methodPresent, methodGet, methodOf, empty);
        } catch (Exception ex) {
            instance = null;
        }
        INSTANCE = instance;
